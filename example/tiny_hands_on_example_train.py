...
if not os.path.exists("tiny_hands_on_example.ttl"):
    print("Loading from inline content since file not found")
    # Copy the content from the provided TTL file
    with open("tiny_hands_on_example.ttl", "w") as f:
        f.write("""@prefix ns1: <http://example.org/> .

ns1:EiffelTower a ns1:Monument ;
    ns1:located_in ns1:Paris .

ns1:Riesenrad a ns1:Monument ;
    ns1:located_in ns1:Vienna .

ns1:Austria a ns1:Country .

ns1:France a ns1:Country .

ns1:Paris a ns1:City ;
    ns1:country ns1:France .

ns1:Vienna a ns1:City ;
    ns1:country ns1:Austria .
""")

# Load the RDF data from the TTL file
g = Graph()
g.parse("tiny_hands_on_example.ttl", format="turtle")
